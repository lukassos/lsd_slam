...
void rgb2bgr(cv::Mat *image) {
    ...
    for (int x = 0; x < image->cols; x++)
        for (int y = 0; y < image->rows; y++)
            image->at<cv::Vec3b>(y, x) = cv::Vec3b(image->at<cv::Vec3b>(y, x)[2], image->at<cv::Vec3b>(y, x)[1],
        ...
        for (int y = 1; y < image->rows - 1; y++) {
            ...
            for (int dx = -1; dx <= 1; dx++)
                for (int dy = -1; dy <= 1; dy++) {
                    ...
                    int c2Val = (c2[0] - c2[1]) * (c2[0] - c2[1]) + (c2[0] - c2[2]) * (c2[0] - c2[2]) +
                                (c2[2] - c2[1]) * (c2[2] - c2[1]);
                    if (c2Val < highTH)
                        continue;
...
int inlayVid3(int argc, char **argv) {
    ...
    printf("pcFiles: %s, odometry: idepth: %s, var: %s, org: %s, age: %s\n",
           std::to_string(pointcloudFiles.size()),
           std::to_string(odometryFilesIdepth.size()),
           std::to_string(odometryFilesVar.size()),
           std::to_string(odometryFilesOrg.size()),
           std::to_string(odometryFilesAge.size())
    );
    ...
    for (size_t i = 0; i < pointcloudFiles.size(); ++i) {
        ...
        if (mindiff > 10)
            printf("pairing %s with %s (diff is: %s)\n",
                   std::to_string(pointcloudTimestamps[i]),
                   std::to_string(odometryTimestamps[mindiffIDX]),
                   std::to_string(mindiff)
            );
...
int inlayVidNew1(int argc, char **argv) {
    ...
    std::vector <string> odometryFilesMapped = getFileList(
            "/home/engelj/fuerte_workspace/bags/videoData/odometry-slam8-02/", "mapped-");
    std::vector <string> odometryFilesTracked = getFileList(
            "/home/engelj/fuerte_workspace/bags/videoData/odometry-slam8-02/", "tracked-");

    std::string target = "/home/engelj/fuerte_workspace/bags/videoData/stitched-slam8";

    printf("pcFiles: %s, mapped: %s, tracked: %s\n",
           std::to_string(pointcloudFiles.size()),
           std::to_string(odometryFilesMapped.size()),
           std::to_string(odometryFilesTracked.size())
    );
    ...
    for (int i = 0; i < pointcloudFiles.size(); i++) {
        ...
        for (int j = 0; j < mappedTimes.size(); j++)
            if ((mappedTimes[j] - (pcTs - fixedMappingOffset)) * (mappedTimes[j] - (pcTs - fixedMappingOffset)) <
                preDiff) {
                mappedIdx = j;
                preDiff =
                        (mappedTimes[j] - (pcTs - fixedMappingOffset)) * (mappedTimes[j] - (pcTs - fixedMappingOffset));
            }
        ...
        snprintf(buf, 1000, "%s/img%05d.png",
                 target.c_str(),
                 i
        );
        cv::imwrite(buf, p);
...
int inlayVid(int argc, char **argv) {
    ...
    printf("pcFiles: %s, odometry: age: %s, idepth: %s, stereo: %s, var: %s\n",
           std::to_string(pointcloudFiles.size()),
           std::to_string(odometryFilesAge.size()),
           std::to_string(odometryFilesIdepth.size()),
           std::to_string(odometryFilesStereo.size()),
           std::to_string(odometryFilesVar.size())
    );
    ...
    for (int i = 0; i < pointcloudFiles.size(); i++) {
        ...
        if (2 != sscanf(tmpString.c_str(), "%ld-%ld.jpg",
                        &id,
                        &time
        ))
            printf("could not parse name as pc %s!\n",
    ...
    for (int i = 0; i < pointcloudFiles.size(); i++) {
        ...
        for (int px = 0; px < oStereo.cols * oStereo.rows; px++) {
            if (oStereo.data[3 * px + 0] == oStereo.data[3 * px + 1] &&
                oStereo.data[3 * px + 0] == oStereo.data[3 * px + 2])
...
int makeBW(int argc, char **argv) {
    ...
    printf("type depth1: %s\n",
           std::to_string(image.type())
    );
    printf("type depth: %s\n",
           std::to_string(imageOut.type())
    );
...
int makeRainbow(int argc, char **argv) {
    ...
    printf("type depth1: %s\n",
           std::to_string(depth1.type())
    );
    printf("type depth: %s\n",
           std::to_string(depth.type())
    );
    printf("type depthC: %s\n",
           std::to_string(depthColor.type())
    );
    ...
    while ('s' != (k = cv::waitKey(0))) {
        ...
        for (int x = 0; x < depth.cols; x++)
            for (int y = 0; y < depth.rows; y++) {
                ...
                if (d == 0)
                    depthColor.at<cv::Vec3b>(y, x) = cv::Vec3b(image.at<uchar>(y, x), image.at<uchar>(y, x),
                                                               image.at<uchar>(y, x));
                else {
                    float id = fac / d;
...
void downsample(std::string folder, int lvl) {
...

    {
        ...
        for (int i = 0; i < d.size(); i++) {
            ...
            printf("depth type %s, u16 %s\n",
                   std::to_string(depth.type()),
                   std::to_string(CV_16U)
            );
