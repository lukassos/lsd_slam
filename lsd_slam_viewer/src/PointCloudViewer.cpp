...
void PointCloudViewer::addFrameMsg(lsd_slam_viewer::keyframeMsgConstPtr msg) {
    ...
    if (!msg->isKeyframe) {
        if (currentCamDisplay->id > msg->id) {
            printf("detected backward-jump in id (%s to %s), resetting!\n",
                   std::to_string(currentCamDisplay->id),
...
void PointCloudViewer::draw() {
    ...
    if (animationPlaybackEnabled) {
        ...
        printf("anim at %s (%s to %s)\n",
               std::to_string(tm),
               std::to_string(kfInt->firstTime()),
               std::to_string(kfInt->lastTime())
        );
        ...
        for (unsigned int i = 0; i < animationList.size(); i++) {
            if (tm >= accTime && tm < accTime + animationList[i].duration && animationList[i].isFix) {
                ...
                printf("fixFrameto %s at %s (%s to %s)\n",
                       std::to_string(i),
                       std::to_string(tm),
                       std::to_string(kfInt->firstTime()),
                       std::to_string(kfInt->lastTime())
    ...
    if (saveAllVideo) {
        ...
        if (span > 0.4) {
            ...
            printf("saved (img %s @ time %s, saveHZ %s)!\n",
                   std::to_string(lastCamID),
                   std::to_string(lastAnimTime),
                   std::to_string(1.0 / localMsBetweenSaves)
            );

            char buf[500];
            snprintf(buf, 500, "%s%s.png",
                     save_folder.c_str(),
...
void PointCloudViewer::remakeAnimation() {
    ...
    printf("made animation with %s keyframes, spanning %s s!\n",
           std::to_string(kfInt->numberOfKeyFrames()),
           std::to_string(tm)
    );
}
...
void PointCloudViewer::keyPressEvent(QKeyEvent *e) {
    switch (e->key()) {
        ...
        case Qt::Key_K:    // add keyframe item
            ...
            animationList.push_back(AnimationObject(false, lastAnimTime, 2, qglviewer::Frame(qglviewer::Vec(0, 0, 0),
        ...
        case Qt::Key_F :    // save list
        {
            ...
            printf("saved animation list (%s items)!\n",
                   std::to_string((int) animationList.size())
            );
        }
            break;
        ...
        case Qt::Key_L :    // load list
        {
            ...
            printf("loaded animation list! (%s items)!\n",
                   std::to_string( (int) animationList.size())
            );
            remakeAnimation();
        }
