...
void KeyFrameGraphDisplay::draw() {
    ...
    if (flushPointcloud) {
        ...
        flushPointcloud = false;
        printf("Done Flushing Pointcloud with %s points!\n",
               std::to_string(numpts)
        );
    ...
    if (printNumbers) {
        ...
        printf("Have %s points, %s keyframes, %s constraints. Displaying %s points.\n",
               std::to_string(totalPoint),
               std::to_string((int) keyframes.size()),
               std::to_string((int) constraints.size()),
               std::to_string(visPoints)
        );
        printNumbers = false;
    }
